...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	'sSTATUS_BEARBEITUNG = oTM.GetFieldForTag(db, "acc!STATUS_BEARBEITUNG")
	'sSTATUS_SATZART = oTM.GetFieldForTag(db, "acc!STATUS_SATZART")
	sSICHERHEIT_ART = oTM.GetFieldForTag(db, "acc!BIH_SICHERHEITENART")
	'sSICHERHEITENART_BEZEICHNUNG = oTM.GetFieldForTag(db, "acc!SICHERHEITENART_BEZEICHNUNG")
	sSICHERHEITENWERT_VERTEILT_JURISTISCH = oTM.GetFieldForTag(db, "acc!BIH_SICHERHEITENWERT_VERTEILT_JURISTISCH")
...
SetCheckpoint "Analysis 1.0 - create Unzulässige Sicherheiten"
	...
	sEqFilter = sSICHERHEITENWERT_VERTEILT_JURISTISCH & " > 0"
	...
	sKundenbürgschaften = oSC.UniqueFileName(sWorkingfolderName & "Unzulässige Sicherheiten.IMD", FINAL_RESULT)
