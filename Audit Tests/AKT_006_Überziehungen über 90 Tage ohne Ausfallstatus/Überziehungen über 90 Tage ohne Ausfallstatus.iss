...
'#Region - result files
' Include the primary input file an all tag names
dim sÜberz90_OhneAusfallstatus as string
Dim sÜberz90_OhneAusfallstatus_per_Person As String
'#End Region
Sub Main()
	...
	call registerResult(sÜberz90_OhneAusfallstatus, FINAL_RESULT, 1)
	call registerResult(sÜberz90_OhneAusfallstatus_per_Person, FINAL_RESULT, 2)
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sTAGE_ÜBERZ_KD_BASEL = oTM.GetFieldForTag(db, "acc!TAGE_UEBERZ_KD_BASEL")
...
function Analysis
SetCheckpoint "Analysis 1.0 - create sÜberz90_OhneAusfallstatus"
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Extraction
	task.IncludeAllFields
	sÜberz90_OhneAusfallstatus = oSC.UniqueFileName(sWorkingfolderName & "Überziehungen über 90 Tage ohne Ausfallstatus.IMD", FINAL_RESULT)
	'task.AddExtraction sÜberz90_OhneAusfallstatus, "", sVR_RATING & " == ""4a"" .AND. " & sTAGE_ÜBERZ_KD_BASEL & " >90"
	task.AddExtraction sÜberz90_OhneAusfallstatus, "", sVR_RATING_NUM & " <= 20 .AND. " & sTAGE_ÜBERZ_KD_BASEL & " >90"
...
SetCheckpoint "Analysis 1.0 - create sÜberz90_OhneAusfallstatus_pro Kunde"
	Set db = Client.OpenDatabase(sÜberz90_OhneAusfallstatus)
	...
	sÜberz90_OhneAusfallstatus_per_Person = oSC.UniqueFileName(sWorkingfolderName & "Überziehungen über 90 Tage ohne Ausfallstatus_pro Kunde.IMD", FINAL_RESULT)
	task.OutputDBName = sÜberz90_OhneAusfallstatus_per_Person
