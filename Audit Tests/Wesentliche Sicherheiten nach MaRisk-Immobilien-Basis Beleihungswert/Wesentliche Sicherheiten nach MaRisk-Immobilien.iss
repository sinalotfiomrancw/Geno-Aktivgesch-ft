...
SetCheckpoint "Analysis 1.0 - create Wesentliche Sicherheiten nach MaRisk-Immobilien"
	...
	sEqFilter = sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" .AND. " & sBELEIHUNGSWERT & "  >  " & sBWBR & " .AND. @AGE(@Date(); @if(@AGE( " & sBELEIHUNGSWERT_PER & ";" & sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM & ") >= 0; " & sBELEIHUNGSWERT_PER & "; " & sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM & " )) >  " & sUEDBR
	...
	sSicherheiten_MaRisk_Immobilien = oSC.UniqueFileName(sWorkingfolderName & "Wesentliche Sicherheiten nach MaRisk-Immobilien-Basis Beleihungswert.IMD", FINAL_RESULT)
