...
'#Region - result files
Dim sChangeOfRVandSafty As String
Dim sHigherRVLowerSafety As String
'#End Region

'#Region - dialog
Dim bUseCritRating As Boolean
Dim bUseCritRVChange As Boolean
Dim sCritRating As String
Dim sCritRVChange As String
'#End Region
Sub Main()
	...
	'Call registerResult(sChangeOfRVandSafty, INTERMEDIATE_RESULT, 1)
	Call registerResult(sHigherRVLowerSafety, FINAL_RESULT,1)
...
SetCheckpoint "Preparation 1.0 - get dialog parameter"
	If oDialogPara.Contains("sCheckB_Rating") Then bUseCritRating = oDialogPara.Item("sCheckB_Rating").Checked
	If oDialogPara.Contains("sCB_Rating") Then sCritRating = GetIndex(oDialogPara.Item("sCB_Rating").Value)
	If oDialogPara.Contains("sCheckB_ChangeOfRiskVolume") Then bUseCritRVChange = oDialogPara.Item("sCheckB_ChangeOfRiskVolume").Checked
	If oDialogPara.Contains("sTB_ChangeOfRiskVolume") Then sCritRVChange = oDialogPara.Item("sTB_ChangeOfRiskVolume").Value
End Function
' --------------------------------------------------------------------------
...
SetCheckpoint "Analysis 1.0 - "
	...
	task.AddFieldToTotal sSUMME_SICHERHEIT_RV_EUR
	sChangeOfRVandSafty = oSC.UniqueFileName(sWorkingFolderPath & "Übersicht Veränderung Risikovolumen und Sicherheitenbestand", INTERMEDIATE_RESULT)
	task.OutputDBName = sChangeOfRVandSafty
...
SetCheckpoint "Analysis 2.0 - "
	Set db = Client.OpenDatabase(sChangeOfRVandSafty)
	Set task = db.TableManagement
	Set field = db.TableDef.NewField
	field.Name = sColumnConspicuous
	field.Description = "wird mit X markiert, wenn Sicherheiten sich verringter und Risikovolumen erhöht ist"
	field.Type = WI_VIRT_CHAR
	field.Equation = "@if(SUMME_SICHERHEIT_RV_EUR_SUMME < @GetPreviousValue(""SUMME_SICHERHEIT_RV_EUR_SUMME"") .AND. " & sGK_KD_RV_EUR & " > @GetPreviousValue(""" & sGK_KD_RV_EUR & """)" & sEQN_SAMECUSTOMER & "; ""X""; """")"
	field.Length = 1
...
SetCheckpoint "Analysis 2.3 - "
	field.Name = "VORHERIGE_SICHERHEITEN"
	field.Description = "SUMME_SICHERHEIT_RV_EUR_SUMME aus vorherigem Zeitpunkt"
	field.Type = WI_VIRT_NUM
	field.Equation = "@if(" & sColumnConspicuous & " = ""X""" & sEQN_SAMECUSTOMER & "; @GetPreviousValue(""SUMME_SICHERHEIT_RV_EUR_SUMME""); 0)"
	field.Decimals = 2
	task.AppendField field
	task.DisableProgressNotification = True
	task.PerformTask
...
Dim sEQN_HigherRVLowerSafety As String
	sEQN_HigherRVLowerSafety = "AUFFÄLLIG = ""X"""
	If bUseCritRating Then sEQN_HigherRVLowerSafety = sEQN_HigherRVLowerSafety & " .AND. " & sVR_RATING_NUM & " >= " & sCritRating
	If bUseCritRVChange Then sEQN_HigherRVLowerSafety = sEQN_HigherRVLowerSafety & " .AND. @abs(ÄNDERUNG_RV) >= @abs(" & sCritRVChange & ")"
SetCheckpoint "Analysis 3.1 - "
	...
	task.AddFieldToInc "SUMME_SICHERHEIT_RV_EUR_SUMME"
	task.AddFieldToInc sGK_KD_RV_EUR
	task.AddFieldToInc sVR_RATING
	task.AddFieldToInc sVR_RATING_NUM
	task.AddFieldToInc "VORHERIGER_ZEITRAUM"
	'task.AddFieldToInc "VORHERIGE_ANZ_SAETZE"
	task.AddFieldToInc "VORHERIGE_SICHERHEITEN"
	task.AddFieldToInc "VORHERIGES_RV"
	task.AddFieldToInc "ÄNDERUNG_RV"
	sHigherRVLowerSafety = oSC.UniqueFileName(sWorkingFolderPath & "erhöhtes Risikovolumen bei verringertem Sicherheitenbestand", FINAL_RESULT)
	task.AddExtraction sHigherRVLowerSafety, "", sEQN_HigherRVLowerSafety
	task.DisableProgressNotification = True
...
SetCheckpoint "Analysis 3.2 - create action field"
	'Set db = Client.OpenDatabase(sHigherRVLowerSafety)
