...
Dim sBELEIHUNGSGRENZE_ALS_BETRAG As String
'#End Region
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sBELEIHUNGSGRENZE_ALS_BETRAG = oTM.GetFieldForTag(db, "acc!SB_BELEIHUNGSGRENZE_ALS_BETRAG")
	sNACHGEWIESENER_WERT_PER = oTM.GetFieldForTag(db, "acc!SB_NACHGEWIESENER_WERT_PER")
	...
	'sBELEIHUNGSWERT_PER = oTM.GetFieldForTag(db, "acc!SB_BELEIHUNGSWERT_PER")
	...
	'sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM = oTM.GetFieldForTag(db, "acc!SB_BELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM")
...
SetCheckpoint "Analysis 1.0 - create Wesentliche Sicherheiten nach MaRisk-Immobilien"
	...
	SEqHardCode = "(( SICHERHEITENART >= ""300"" .AND. SICHERHEITENART < ""400"" ) .OR. (SICHERHEITENART >= ""500"" .AND. SICHERHEITENART < ""600"" ))"
	
	sBWBR = oParameters.Item("smartTextBox1").Value
	sUEDBR = oParameters.Item("smartTextBox2").Value
	
	sEqFilter = SEqHardCode & ".AND." & sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" .AND. " & sBELEIHUNGSGRENZE_ALS_BETRAG & "  >  " & sBWBR & " .AND. @AGE(@Date(); " & sNACHGEWIESENER_WERT_PER & ") >  " & sUEDBR
	...
	sSicherheiten_MaRisk_Immobilien = oSC.UniqueFileName(sWorkingfolderName & "Wesentliche Sicherheiten nach MaRisk-Sicherungsübereignungen und Zessionen.IMD", FINAL_RESULT)
