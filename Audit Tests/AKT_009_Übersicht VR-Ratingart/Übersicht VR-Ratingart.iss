...
'#Region - result files
' Include the primary input file an all tag names
Dim sÜbersicht_Ratingarten As String
Dim sÜbersicht_Kundennummern As String
Dim sÜbersicht_Ausfallkunden As String
Dim sÜbersicht_RatingartenJeKunde As String
...
Sub Main()
	...
	'Call registerResult(sÜbersicht_Ratingarten, FINAL_RESULT, 1)
	'Call registerResult(sÜbersicht_Kundennummern, FINAL_RESULT,1)
	Call registerResult(sÜbersicht_Ausfallkunden, FINAL_RESULT, 1)
	Call registerResult( sÜbersicht_RatingartenJeKunde , FINAL_RESULT, 1)
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sBONITÄTSEINSTUFUNG = oTM.GetFieldForTag(db, "acc!BONITAETSEINSTUFUNG")
...
Function Analysis
SetCheckpoint "Analysis 1.0 - create sGew_Kreditn_mit_Branken_33"
	'Set db = Client.OpenDatabase(sInputFile)
	'Set task = db.Summarization
	'task.AddFieldToSummarize sVR_RATINGART
	'task.AddFieldToTotal sRISIKOVOLUMEN_EUR
	'task.Criteria= ".Not.(" & sAUSFALLRATE_KUNDE & "= 100)"   'sogenannte Ausfall-Kunden werden rausgefiltert s. Mail von Brummel vom 11.04
	'sÜbersicht_Ratingarten = oSC.UniqueFileName(sWorkingfolderName & "Übersicht Ratingart.IMD", FINAL_RESULT)
	'task.OutputDBName = sÜbersicht_Ratingarten
	...
	'Sog. Ausfallkunden werden gesondert ausgegeben
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Summarization
	task.AddFieldToSummarize sKUNDENNUMMER
	task.AddFieldToInc sKUNDENNAME
	task.AddFieldToInc sRISIKOGRUPPE
	task.AddFieldToInc sBONITÄTSEINSTUFUNG
	task.AddFieldToInc sVR_RATINGART
	task.AddFieldToInc sVR_RATING
	task.AddFieldToInc sDATUM_DATENABZUG
	task.AddFieldToTotal sRISIKOVOLUMEN_EUR
	task.Criteria= sAUSFALLRATE_KUNDE & "= 100"   'sogenannte Ausfall-Kunden werden angezeigt s.o.
	sÜbersicht_Ausfallkunden = oSC.UniqueFileName(sWorkingfolderName & "Ausfallkunden (gesonderte Ausgabe).IMD", Final_RESULT)
	task.OutputDBName = sÜbersicht_Ausfallkunden  
	task.CreatePercentField = FALSE
	...
	'17.07.2023: Gleich Analyse aber zur Basis werden indiv. Kundennummern genommen.
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Summarization
	task.AddFieldToSummarize sKUNDENNUMMER
	task.AddFieldToInc sKUNDENNAME
	...
	task.AddFieldToTotal sRISIKOVOLUMEN_EUR
	task.Criteria= ".Not.(" & sAUSFALLRATE_KUNDE & "= 100)"   'sogenannte Ausfall-Kunden werden rausgefiltert s. Mail von Brummel vom 11.04
	sÜbersicht_Kundennummern = oSC.UniqueFileName(sWorkingfolderName & "{Verdichtet über KD-Nr}.IMD")
	task.OutputDBName = sÜbersicht_Kundennummern 
	task.CreatePercentField = FALSE
	task.UseFieldFromFirstOccurrence = TRUE
	task.StatisticsToInclude = SM_SUM
	task.PerformTask
	...
	Set db = Client.OpenDatabase(sÜbersicht_Kundennummern)
	Set task = db.Summarization
	task.AddFieldToSummarize sVR_RATINGART
	task.AddFieldToInc sDATUM_DATENABZUG
	task.AddFieldToTotal "RISIKOVOLUMEN_EUR_SUMME"
	sÜbersicht_RatingartenJeKunde  = oSC.UniqueFileName(sWorkingfolderName & "Übersicht Ratingart auf Kundenbasis.IMD", Final_RESULT)
	task.OutputDBName = sÜbersicht_RatingartenJeKunde 
	task.CreatePercentField = FALSE
	task.StatisticsToInclude = SM_SUM
	task.PerformTask
	Set task = Nothing
