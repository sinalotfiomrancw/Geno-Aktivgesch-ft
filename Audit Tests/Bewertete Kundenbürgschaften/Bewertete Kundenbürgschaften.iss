...
Dim sSICHERHEIT_ART_Num As String
'#End Region

'#Region Analysis and Result
Dim sKundenbürgschaften As String
Dim sKundenbürgschaften2 As String
...
Sub Main()
	...
	Call registerResult(sKundenbürgschaften, FINAL_RESULT, 1)
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sSTATUS_BEARBEITUNG = oTM.GetFieldForTag(db, "acc!SBH_STATUS_BEARBEITUNG")
	sSTATUS_SATZART = oTM.GetFieldForTag(db, "acc!SBH_STATUS_SATZART")
	sSICHERHEIT_ART = oTM.GetFieldForTag(db, "acc!SBH_SICHERHEITENART")
	...
	sVERFUEGBARER_SICHERUNGSWERT = oTM.GetFieldForTag(db, "acc!SBH_VERFUEGBARER_SICHERUNGSWERT")
	'sSICHERHEIT_ART_Num = oTM.GetFieldForTag(db, "acc!SBH_SICHERHEITENART_NUMERISCH")
	sSICHERHEIT_ART_Num = "@Val(" & sSICHERHEIT_ART & ")"
...
SetCheckpoint "Analysis 1.0 - create  Einhaltung der Beleihungsgrenzen"
	...
	SEqHardCode = "( SICHERHEITENART >= ""211"" .AND. SICHERHEITENART <= ""255"" )"
	
	sEqFilter = sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" .AND. " & sVERFUEGBARER_SICHERUNGSWERT & " > 0" & " .AND. " & SEqHardCode
	...
	sKundenbürgschaften = oSC.UniqueFileName(sWorkingfolderName & "Bewertete Kundenbürgschaften.IMD", FINAL_RESULT)
	task.AddExtraction sKundenbürgschaften, "", sEqFinal
...
SetCheckpoint "BuildEquation_FromToList_0"
	...
	accountNumberName = sSICHERHEIT_ART_Num
