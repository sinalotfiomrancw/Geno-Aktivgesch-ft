...
SetCheckpoint "Preparation 1.0 - get dialog parameter"
	...
	'---------------------------------------------
	sEQN = sVR_RATING_NUM  & " >= 22 .AND. " & sVR_RATING_NUM  & " <> 99 .AND. " & sEWB_RST_GEBUCHT_EUR & " = 0"
	'---------------------------------------------
	sEQN_EWB = sEQN & " .AND. " & sBLANKOVOLUMEN_IA_EUR & " > 0"
	sEQN_RST = sEQN & " .AND. " & sBLANKOVOLUMEN_RV_EUR & " > 0"
	sEQN_EWB_RST = sEQN & " .AND. " & sBLANKOVOLUMEN_IA_EUR & " > 0" & " .AND. " & sBLANKOVOLUMEN_RV_EUR & " > 0"
	'---------------------------------------------
	if sCritBlankRiskVolume <> "" then
		sEQN_EWB = sEQN_EWB & " .AND. " & sBLANKOVOLUMEN_IA_EUR & " >= " & sCritBlankRiskVolume
		sEQN_RST = sEQN_RST & " .AND. " & sBLANKOVOLUMEN_IA_EUR & " >= " & sCritBlankRiskVolume
		sEQN_EWB_RST = sEQN_EWB_RST & " .AND. " & sBLANKOVOLUMEN_IA_EUR & " >= " & sCritBlankRiskVolume
	end if
End Function
' --------------------------------------------------------------------------
...
Function Analysis
SetCheckpoint "Analysis 1.0 - create sKreditnehmMitAusfall_OhneEWB"
	Set db = Client.OpenDatabase(sInputFile)
	...
	'task.AddExtraction sKreditnehmMitAusfall_OhneEWB, "", sVR_RATING & " == ""4a"" .AND. " & sEWB_RST_GEBUCHT_EUR & " = 0"
	'task.AddExtraction sKreditnehmMitAusfall_OhneEWB, "", sVR_RATING_NUM  & " >= 22 .AND. " & sEWB_RST_GEBUCHT_EUR & " = 0" ' 99 Ratings rauswerfen!
	'task.AddExtraction sKreditnehmMitAusfall_OhneEWB, "", sVR_RATING_NUM  & " >= 22 .AND. " & sVR_RATING_NUM  & " <> 99 .AND. " & sEWB_RST_GEBUCHT_EUR & " = 0" ' 22.08.2023
	task.AddExtraction sKreditnehmMitAusfall_OhneEWB, "", sEQN_EWB ' 11.09.2023
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
' 20.10.2023 AS
SetCheckpoint "Analysis 2.0 - create sKreditnehmMitAusfall_OhneRST"
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Extraction
	task.IncludeAllFields
	...
	task.AddExtraction sKreditnehmMitAusfall_OhneRST, "", sEQN_RST
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
SetCheckpoint "Analysis 3.0 - create sKreditnehmMitAusfall_OhneEWBundRST"
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Extraction
	task.IncludeAllFields
	...
	task.AddExtraction sKreditnehmMitAusfall_OhneEWBundRST, "", sEQN_EWB_RST
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
