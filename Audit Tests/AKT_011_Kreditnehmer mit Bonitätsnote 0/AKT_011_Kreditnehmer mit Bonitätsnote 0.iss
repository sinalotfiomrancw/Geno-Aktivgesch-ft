...
Function Analysis
SetCheckpoint "Analysis 1.0 - create sKreditn_Bonität_0"
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Extraction
	task.IncludeAllFields
	sKreditn_Bonität_0 = oSC.UniqueFileName(sWorkingfolderName & "Kreditnehmer mit Bonitätsnote 0.IMD", FINAL_RESULT)
	task.AddExtraction sKreditn_Bonität_0, "", sBONITÄTSEINSTUFUNG & " == ""0"" .AND." & sAUSFALLRATE_KUNDE & "<> 100" 'zweiter Teil hinzugefügt, um sog. Ausfal-Kunden aus der Statistik rauszunhmen. s. Mail von Brummel vom 11.04.23
	...
	Set task = db.Extraction
	task.IncludeAllFields
	sKreditn_Ausfallkunden = oSC.UniqueFileName(sWorkingfolderName & "Ausfallkunden mit Bonitätsnote 0.IMD", FINAL_RESULT)
	task.AddExtraction sKreditn_Ausfallkunden, "", sBONITÄTSEINSTUFUNG & " == ""0"" .AND." & sAUSFALLRATE_KUNDE & "== 100" 
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
