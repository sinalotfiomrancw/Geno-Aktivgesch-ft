...
Dim sBeleihungswertermittlungen_bei_Immobilien As String
Dim sBeleihungswertermittlungen_bei_Immobilien_Filtered As String
Dim sBeleihungswertermittlungen_bei_Immobilien_A As String
Dim sBeleihungswertermittlungen_bei_Immobilien_Grp_Immo As String
...
Sub Main()
	...
	Call registerResult(sBeleihungswertermittlungen_bei_Immobilien, FINAL_RESULT, 1)
	Call registerResult(sBeleihungswertermittlungen_bei_Immobilien_Grp_Immo, FINAL_RESULT, 1)
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sBELEIHUNGSGRENZE_BETRAG = oTM.GetFieldForTag(db, "acc!SBI_BELEIHUNGSGRENZE_IN_EURO")
	...
	sBELEIHUNGSWERT_STATUS = oTM.GetFieldForTag(db, "acc!SBI_BELEIHUNGSWERT_STATUS")
...
SetCheckpoint "Analysis 1.0 - create Vorläufige Beleihungswertermittlungen bei Immobilien"
	...
	If Len(sdatefrom) > 0 Then  
		sEqn_A = "(" & sBELEIHUNGSWERT_STATUS_PER & " <= @ctod(""" & sdatefrom & """" & sLS & """YYYYMMDD""))" 
	End If
	...
	sEqFilter = sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" " & " .AND. " & sBELEIHUNGSGRENZE_BETRAG & " > 0 " & " .AND. " & "(" & sBELEIHUNGSWERT_STATUS & " = ""2"" .OR. " & sBELEIHUNGSWERT_STATUS & " = ""3"" " & ")"
	
	sEqFinal = sEqn_0 & " .AND. " & sEqFilter & " .AND. " & sEqn_A
	...
	sBeleihungswertermittlungen_bei_Immobilien = oSC.UniqueFileName(sWorkingfolderName & "Vorläufige Beleihungswertermittlungen bei Immobilien.IMD", FINAL_RESULT)
	task.AddExtraction sBeleihungswertermittlungen_bei_Immobilien, "", sEqFinal
	...
	Set db = Client.OpenDatabase(sBeleihungswertermittlungen_bei_Immobilien)
	...
	sBeleihungswertermittlungen_bei_Immobilien_Grp_Immo = oSC.UniqueFileName(sWorkingfolderName & "Vorläufige Beleihungswertermittlungen bei Immobilien_pro Immobilien Nr.IMD", FINAL_RESULT)
	task.OutputDBName = sBeleihungswertermittlungen_bei_Immobilien_Grp_Immo
...
SetCheckpoint "BuildEquation_FromToList_2"	
		...
		err.Raise 16, "AKT-031", ErrorMessageFilterLength
