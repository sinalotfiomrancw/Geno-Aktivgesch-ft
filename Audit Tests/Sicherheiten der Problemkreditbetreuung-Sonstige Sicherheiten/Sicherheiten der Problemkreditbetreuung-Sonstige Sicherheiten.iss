...
Dim sProblemkreditbetreuung_Sonstige_Sicherheiten As String
...
Sub Main()
	...
	Call registerResult(sProblemkreditbetreuung_Sonstige_Sicherheiten, FINAL_RESULT, 1)
...
SetCheckpoint "GetFileInformation 1.0 - get primary input file"
	...
	'sAlias = "Zweckerklärungen-Si-Wert"
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sSTATUS_BEARBEITUNG = oTM.GetFieldForTag(db, "acc!BAS_STATUS_BEARBEITUNG")
	sSTATUS_SATZART = oTM.GetFieldForTag(db, "acc!BAS_STATUS_SATZART")
	sSICHERHEIT_ART = oTM.GetFieldForTag(db, "acc!BAS_SICHERHEITENART")
	...
	sBELEIHUNGSWERT = oTM.GetFieldForTag(db, "acc!BAS_BELEIHUNGSWERT")
	...
	sNACHGEWIESENER_WERT_PER = oTM.GetFieldForTag(db, "acc!BAS_NACHGEWIESENER_WERT_PER")
	...
	sRISIKOSTATUS_MARISK = oTM.GetFieldForTag(db, "acc!BAS_RISIKOSTATUS_MARISK")
	sRISIKOSTATUS_MARISK_SEIT = oTM.GetFieldForTag(db, "acc!BAS_RISIKOSTATUS_MARISK_SEIT")
...
SetCheckpoint "Analysis 1.0 - create Sicherheiten der Problemkreditbetreuung - Sonstige Sicherheiten"
	...
	sEqFilter = sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" .AND. (" & sRISIKOSTATUS_MARISK & " = ""4"" .OR. " & sRISIKOSTATUS_MARISK & " = ""5"") .AND. " & sBELEIHUNGSWERT & "  >  " & sBWBR & " .AND. " & sRISIKOSTATUS_MARISK_SEIT & " > " & sNACHGEWIESENER_WERT_PER
	...
	'sSBI_Join_SZS = oSC.UniqueFileName(sWorkingfolderName & "Si-Basisdaten-Immo verknüpft mit Sicherheiten-Zweckerklärungen-Si-Wert.IMD", NOT_A_RESULT)
	...
	sProblemkreditbetreuung_Sonstige_Sicherheiten = oSC.UniqueFileName(sWorkingfolderName & "Sicherheiten der Problemkreditbetreuung-Sonstige Sicherheiten.IMD", FINAL_RESULT)
	task.AddExtraction sProblemkreditbetreuung_Sonstige_Sicherheiten, "", sEqFinal
