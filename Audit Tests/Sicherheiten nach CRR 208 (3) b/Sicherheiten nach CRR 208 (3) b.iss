...
Dim sSZR_ANTEIL_SICHERUNGSWERT_GEMAESS_BERECH As String
Dim sSZR_PERSONENNUMMER_SICHERHEITENNEHMER As String
Dim sSZR_PERSONENNUMMER_SICHERHEITENGEBER  As String
Dim sSZR_SICHERHEITENNUMMER As String
Dim sANTEIL_SICHERUNGSWERT_GEMAESS_BERECH As String
'#End Region

'#Region Analysis and Result
Dim sSicherheiten_nach_CRR As String
Dim sSicherheitenart As String
Dim sKSBW As String
Dim sUEDBR As String
Dim sSBI_Join_SZR As String
Dim sFileName_SZR As String
'#Region Analysis and Result
...
Sub Main()
	...
	Call registerResult(sSicherheiten_nach_CRR, FINAL_RESULT, 1)
...
SetCheckpoint "GetFileInformation 1.0 - get primary input file"
	...
	sAlias = "Zweckerkl‰rungen-Realkredit"
	If SmartContext.InputFilesContainsAlias(sAlias) Then
		sFileName_SZR = SmartContext.InputFileByAlias(sAlias)
	End If
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sSTATUS_BEARBEITUNG = oTM.GetFieldForTag(db, "acc!RIM_STATUS_BEARBEITUNG")
	sSTATUS_SATZART = oTM.GetFieldForTag(db, "acc!RIM_STATUS_SATZART")
	sSICHERHEIT_ART = oTM.GetFieldForTag(db, "acc!RIM_SICHERHEITENART")
	...
	sBELEIHUNGSWERT_PER = oTM.GetFieldForTag(db, "acc!RIM_BELEIHUNGSWERT_PER")
	...
	sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM = oTM.GetFieldForTag(db, "acc!RIM_BELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM")
	sANTEIL_SICHERUNGSWERT_GEMAESS_BERECH = oTM.GetFieldForTag(db, "acc!RIM_SUMME_SI_WERT_JE_SICHERHEITENNEHMER")
...
Dim sTempDB1 As String
Dim sTempDB2 As String
SetCheckpoint "Analysis 1.0 - create Sicherheiten nach CRR 208"
	...
	sKSBW = oParameters.Item("smartTextBox1").Value
	sUEDBR = oParameters.Item("smartTextBox2").Value
	
	sEqFilter = sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" .AND. " & " @AGE(@Date(); @if(@AGE( " & sBELEIHUNGSWERT_PER & ";" & sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM & ") >= 0; " & sBELEIHUNGSWERT_PER & "; " & sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM & " )) >  " & sUEDBR & ".AND." & sANTEIL_SICHERUNGSWERT_GEMAESS_BERECH & " > " & sKSBW
...
'	Set db = Client.OpenDatabase("{Sicherheiten_Zweckerkl‰rungen_Realkedit}.IMD")
'	Set task = db.Summarization
'	task.AddFieldToSummarize "PERSONENNUMMER_SICHERHEITENNEHMER"
'	task.AddFieldToTotal "ANTEIL_SICHERUNGSWERT_GEMƒﬂ_BERECHNUNGSA"
'	dbName = "Felder aufsummieren1.IMD"
...
'	task.FileToJoin "Felder aufsummieren1.IMD"
'	task.IncludeAllPFields
'	task.AddSFieldToInc "ANTEIL_SICHERUNGSWERT_GEMƒﬂ_BERECHNUNGSA"
'	task.AddMatchKey "PERSONENNUMMER_SICHERHEITENNEHMER", "PERSONENNUMMER_SICHERHEITENNEHMER", "A"
'	dbName = "Dateien verbinden.IMD"
'		task.PerformTask dbName, "", WI_JOIN_ALL_IN_PRIM
'	Set task = Nothing
'	Set db = Nothing
'	Client.OpenDatabase (dbName)

'	Set db = Client.OpenDatabase("Dateien verbinden.IMD")
'	Set task = db.JoinDatabase
'	task.FileToJoin "{Sicherheiten_Basisdaten_Immo}.IMD"
'	task.IncludeAllPFields
...
'	Set db = Client.OpenDatabase(sInputFile)
'	Set task = db.JoinDatabase
'	task.FileToJoin sFileName_SZR
'	task.IncludeAllPFields
'	task.AddSFieldToInc sSZR_ANTEIL_SICHERUNGSWERT_GEMAESS_BERECH
'	task.AddSFieldToInc sSZR_PERSONENNUMMER_SICHERHEITENNEHMER
'	task.AddMatchKey "PERSONENNUMMER", "PERSONENNUMMER_SICHERHEITENGEBER", "A"
'	task.AddMatchKey "SICHERHEITENNUMMER", "SICHERHEITENNUMMER", "A"
'	sSBI_Join_SZR = oSC.UniqueFileName(sWorkingfolderName & "Si-Basisdaten-Immo verkn¸pft mit Si-Zweckerkl‰rungen-Realkreditt.IMD", NOT_A_RESULT)
'	task.PerformTask sSBI_Join_SZR, "", WI_JOIN_MATCH_ONLY
'	db.Close
'	Set task = Nothing
'	Set db = Nothing
'	
...
'	Set db = Client.OpenDatabase(sSBI_Join_SZR)
'	Set task = db.Extraction
'	task.IncludeAllFields
'	sTempDB1 = oSC.UniqueFileName(sWorkingfolderName & "Sicherheiten nach CRR 208_gefiltert nach Eingabe.IMD", NOT_A_RESULT)
'	task.AddExtraction sTempDB1, "", sEqFinal
'	task.PerformTask 1, db.Count
'	db.Close
'	Set task = Nothing
'	Set db = Nothing
'	
'	Set db = Client.OpenDatabase(sTempDB1)
'	Set task = db.Summarization
'	task.AddFieldToSummarize sSZR_PERSONENNUMMER_SICHERHEITENNEHMER
'	task.AddFieldToTotal "ANTEIL_SICHERUNGSWERT_GEMƒﬂ_BERECHNUNGSA"
'	task.IncludeAllFields
'	sTempDB2 = oSC.UniqueFileName(sWorkingfolderName & "Sicherheiten nach CRR 208_gefiltert nach Eingabe und je Person aufsummiert.IMD", NOT_A_RESULT)
'	task.OutputDBName = sTempDB2
'	task.CreatePercentField = FALSE
...
'	Set db = Nothing
'	
'	sEqFinal = " ANTEIL_SICHERUNGSWERT_GEMƒﬂ_BERECHNUNGSA > " & sKSBW
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Extraction
	task.IncludeAllFields
	sSicherheiten_nach_CRR = oSC.UniqueFileName(sWorkingfolderName & "Sicherheiten nach CRR 208 (3) b.IMD", FINAL_RESULT)
	task.AddExtraction sSicherheiten_nach_CRR, "", sEqFinal
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
...
SetCheckpoint "BuildEquation_FromToList_2"	
		...
		ErrorMessageFilterLength = "√úberpr√ºfen Sie die Liste der Sachkontonummernbereiche. Ein Filterausdruck, der die f√úr IDEA festgelegte maximale L√É¬§nge einer Gleichung √úberschreitet, kann nicht generiert werden."
