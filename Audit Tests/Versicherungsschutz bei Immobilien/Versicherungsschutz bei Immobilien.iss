...
Dim sDIE_QUALITATIVEN_ANFORDERUNGEN_SIND_ERFUE As String
'#End Region

'#Region Analysis and Result
Dim sKundenbürgschaften As String
Dim sKundenbürgschaften_A As String
Dim sKundenbürgschaften_B As String
...
Sub Main()
	...
	Call registerResult(sKundenbürgschaften_A, FINAL_RESULT, 1)
	Call registerResult(sKundenbürgschaften_B, FINAL_RESULT, 1)
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sFEUERVERSICHERUNGSSCHEIN_NR = oTM.GetFieldForTag(db, "acc!SBI_FEUERVERSICHERUNGSSCHEIN_NR")
	sDIE_QUALITATIVEN_ANFORDERUNGEN_SIND_ERFUE = oTM.GetFieldForTag(db, "acc!SBI_DIE_QUALITATIVEN_ANFORDERUNGEN_SIND_")
	db.Close
	Set db = Nothing
end function
' --------------------------------------------------------------------------
...
Dim sEqFinal_A As String
Dim sEqFinal_B As String

SetCheckpoint "Analysis 1.0 - create Versicherungsschutz bei Immobilien"
	...
	sEqFilter = sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" .AND. " & sBELEIHUNGSWERT & " > " & sBWBR 
	
	sEqFinal = sEqn_0 & " .AND. " & sEqFilter
	
	SmartContext.Log.LogMessage "Final Equation = {0}", sEqFinal
	
	sEqFinal_A = sEqFinal & " .AND. " & sFEUERVERSICHERUNGSSCHEIN_NR & " = """" " 
	SmartContext.Log.LogMessage "Final Equation_A = {0}", sEqFinal_A
	sEqFinal_B = sEqFinal & " .AND. " & sFEUERVERSICHERUNGSSCHEIN_NR & " = """" " & " .AND. " & sDIE_QUALITATIVEN_ANFORDERUNGEN_SIND_ERFUE & " = ""J"""
	SmartContext.Log.LogMessage "Final Equation_B = {0}", sEqFinal_B

	Set oParameters = Nothing
	Set db = Client.OpenDatabase(sInputFile)
	Set task = db.Extraction
	task.IncludeAllFields
	sKundenbürgschaften_A = oSC.UniqueFileName(sWorkingfolderName & "Versicherungsschutz bei Immobilien-ohne Feuerversicherungsschein Nr.IMD", FINAL_RESULT)
	task.AddExtraction sKundenbürgschaften_A, "", sEqFinal_A
	...
	Set task = db.Extraction
	task.IncludeAllFields
	sKundenbürgschaften_B = oSC.UniqueFileName(sWorkingfolderName & "Versicherungsschutz bei Immobilien-ohne Feuerversicherungsschein Nr und mit erfüllten qualitativen Anforderungen.IMD", FINAL_RESULT)
	task.AddExtraction sKundenbürgschaften_B, "", sEqFinal_B
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
