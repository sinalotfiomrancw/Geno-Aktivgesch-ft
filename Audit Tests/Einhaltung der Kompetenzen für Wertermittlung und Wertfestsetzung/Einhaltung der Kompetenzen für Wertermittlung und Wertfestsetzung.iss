...
Dim sEinhaltung_Kompetenzen As String
Dim sEinhaltung_Kompetenzen_Filtered As String
Dim sEinhaltung_Kompetenzen_A As String
Dim sSicherheitenart As String
Dim sBWBR As String
Dim sUEDBR As String
Dim sRatingNum As String
Dim sdatefrom As String
Dim sEinhaltung_Kompetenzen_B As String
Dim sEinhaltung_Kompetenzen_A_je_Genehmiger As String
Dim sEinhaltung_Kompetenzen_A_je_Übpf As String
Dim sEinhaltung_Kompetenzen_B_je_Genehmiger As String
Dim sEinhaltung_Kompetenzen_B_je_Übpf As String
...
Sub Main()
	...
	IgnoreWarning(True)
	sRatingNum = "Rating_Num"
	Call GetFileInformation
	Call Analysis
	Call registerResult(sInputFile, INPUT_DATABASE, 0)
	Call registerResult(sEinhaltung_Kompetenzen_A, FINAL_RESULT, 1)
	Call registerResult(sEinhaltung_Kompetenzen_A_je_Genehmiger, FINAL_RESULT, 1)
	Call registerResult(sEinhaltung_Kompetenzen_A_je_Übpf, FINAL_RESULT, 1)
	Call registerResult(sEinhaltung_Kompetenzen_B, FINAL_RESULT, 1)
	Call registerResult(sEinhaltung_Kompetenzen_B_je_Genehmiger, FINAL_RESULT, 1)
	Call registerResult(sEinhaltung_Kompetenzen_B_je_Übpf, FINAL_RESULT, 1)
...
SetCheckpoint "GetFileInformation 2.0 - get tags"
	...
	sBELEIHUNGSWERT_STATUS_PER = oTM.GetFieldForTag(db, "acc!SBI_BELEIHUNGSWERT_STATUS_PER")
	...
	sEINGETRAGENER_BETRAG_DER_GRUNDSCHULD_NOM = oTM.GetFieldForTag(db, "acc!SBI_EINGETRAGENER_BETRAG")
...
Dim sEqn_A As String
Dim sEqn_B As String
Dim sEqnReserved As String
Dim sEqFilter As String
Dim sEqFinal As String
SetCheckpoint "Analysis 1.0 - create Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung"
	...
	If oParameters.Contains("smartFromToList1") Then
		sSicherheitenart = BuildEquation_FromToList(sInputFile)
	End If
	...
	sEqFilter = sSTATUS_BEARBEITUNG & " = ""A"" .AND. " & sSTATUS_SATZART & " = ""J"" "
	...
	sEinhaltung_Kompetenzen = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung_Sicherheitenart filtered.IMD", FINAL_RESULT)
	task.AddExtraction sEinhaltung_Kompetenzen, "", sEqFinal
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
	
	Set db = Client.OpenDatabase(sEinhaltung_Kompetenzen)
	Set task = db.Summarization
	task.AddFieldToSummarize sIMMOBILIEN_NR
	task.AddFieldToInc "PERSONENNUMMER"
	task.AddFieldToInc "NACHNAME_KURZ"
	'task.AddFieldToInc "SICHERHEITENNUMMER"
	'task.AddFieldToInc "STATUS_BEARBEITUNG"
	'task.AddFieldToInc "STATUS_SATZART"
	'task.AddFieldToInc "SICHERHEITENART"
	'task.AddFieldToInc "SICHERHEITENART_BEZEICHNUNG"
	'task.AddFieldToInc "EIGENTÜMER_PERSONENNUMMER"
	'task.AddFieldToInc "FEUERVERSICHERUNGSSCHEIN_NR"
	'task.AddFieldToInc "BEWERTUNGSART"
	...
	task.AddFieldToInc "BELEIHUNGSWERT_STATUS_VON"
	task.AddFieldToInc "BELEIHUNGSWERT_STATUS_NUMMER_DES_BEDIENE"
	task.AddFieldToInc "BELEIHUNGSWERT_WURDE_ÜBERPRÜFT_AM"
	task.AddFieldToInc "BELEIHUNGSWERT_WURDE_ÜBERPRÜFT_VON"
	...
	task.AddFieldToTotal sEINGETRAGENER_BETRAG_DER_GRUNDSCHULD_NOM
	sEinhaltung_Kompetenzen_Filtered = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung_Summe pro Immobilien Nr.IMD", FINAL_RESULT)
	task.OutputDBName = sEinhaltung_Kompetenzen_Filtered
	...
	Call CreateNewField(sEinhaltung_Kompetenzen_Filtered)
	...
	If Len(sdatefrom) > 0 Then  
		...
		sEqn_1 = "(" & "JÜNGSTE_DATUM" & " >= @ctod(""" & sdatefrom & """" & sLS & """YYYYMMDD""))" 
		
	End If
	
	sEqn_A = "ERMITTLUNG_KLEINDARLEHENSGRENZE = ""Ja"" .AND." & sEqn_1
	SmartContext.Log.LogMessage "Final Equation_A = {0}", sEqn_A
	sEqn_B = "ERMITTLUNG_KLEINDARLEHENSGRENZE = ""Nein"" .AND." & sEqn_1
	SmartContext.Log.LogMessage "Final Equation_B = {0}", sEqn_B
	
	Set db = Client.OpenDatabase(sEinhaltung_Kompetenzen_Filtered)
	Set task = db.Extraction
	task.IncludeAllFields
	sEinhaltung_Kompetenzen_A = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung-Kleindarlehensgrenze.IMD", FINAL_RESULT)
	task.AddExtraction sEinhaltung_Kompetenzen_A, "", sEqn_A
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
	
	Set db = Client.OpenDatabase(sEinhaltung_Kompetenzen_A)
	Set task = db.Summarization
	task.AddFieldToSummarize "BELEIHUNGSWERT_STATUS_VON"
	sEinhaltung_Kompetenzen_A_je_Genehmiger = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung-Kleindarlehensgrenze-Anzahl der Immobilien-Nrn. je Genehmiger.IMD", FINAL_RESULT)
	task.OutputDBName = sEinhaltung_Kompetenzen_A_je_Genehmiger
	task.CreatePercentField = FALSE
	task.PerformTask
	db.Close
	Set task = Nothing
	...
	Set db = Client.OpenDatabase(sEinhaltung_Kompetenzen_A)
	Set task = db.Summarization
	task.AddFieldToSummarize "BELEIHUNGSWERT_WURDE_ÜBERPRÜFT_VON"
	sEinhaltung_Kompetenzen_A_je_Übpf = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung-Kleindarlehensgrenze-Anzahl der Immobilien-Nrn. je Überprüfer.IMD", FINAL_RESULT)
	task.OutputDBName = sEinhaltung_Kompetenzen_A_je_Übpf
	task.CreatePercentField = FALSE
	task.PerformTask
	db.Close
	Set task = Nothing
	...
	Set task = db.Extraction
	task.IncludeAllFields
	sEinhaltung_Kompetenzen_B = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung-keine Kleindarlehensgrenze.IMD", FINAL_RESULT)
	task.AddExtraction sEinhaltung_Kompetenzen_B, "", sEqn_B
	task.PerformTask 1, db.Count
	db.Close
	Set task = Nothing
	Set db = Nothing
	
	Set db = Client.OpenDatabase(sEinhaltung_Kompetenzen_B)
	Set task = db.Summarization
	task.AddFieldToSummarize "BELEIHUNGSWERT_STATUS_VON"
	sEinhaltung_Kompetenzen_B_je_Genehmiger = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung-keine Kleindarlehensgrenze-Anzahl der Immobilien-Nrn. je Genehmiger.IMD", FINAL_RESULT)
	task.OutputDBName = sEinhaltung_Kompetenzen_B_je_Genehmiger
	task.CreatePercentField = FALSE
	task.PerformTask
	db.Close
	Set task = Nothing
	...
	Set task = db.Summarization
	task.AddFieldToSummarize "BELEIHUNGSWERT_WURDE_ÜBERPRÜFT_VON"
	sEinhaltung_Kompetenzen_B_je_Übpf = oSC.UniqueFileName(sWorkingfolderName & "Einhaltung der Kompetenzen für Wertermittlung und Wertfestsetzung-keine Kleindarlehensgrenze-Anzahl der Immobilien-Nrn. je Überprüfer.IMD", FINAL_RESULT)
	task.OutputDBName = sEinhaltung_Kompetenzen_B_je_Übpf
	task.CreatePercentField = FALSE
	task.PerformTask
	db.Close
	Set task = Nothing
...
Function CreateNewField(ByVal dbNameInput As String)

Set db = Client.OpenDatabase(dbNameInput)
Set task = db.TableManagement
Set field = db.TableDef.NewField
field.Name = "ERMITTLUNG_KLEINDARLEHENSGRENZE"
field.Description = "Hinzugefügtes Feld"
field.Type = WI_VIRT_CHAR
field.Equation = "@If( " & sEINGETRAGENER_BETRAG_DER_GRUNDSCHULD_NOM & "_SUMME" & " < 600000   .AND.  ( " & sNUTZUNGSART & " = ""2""  .OR. " & sNUTZUNGSART &  " = ""7""); ""Ja""; ""Nein"")"
field.Length = 4
...
Set task = db.TableManagement
Set field = db.TableDef.NewField
field.Name = "JÜNGSTE_DATUM_PER_AM"
field.Description = "Hinzugefügtes Feld"
field.Type = WI_VIRT_DATE
field.Equation = "@if(@AGE(" & sBELEIHUNGSWERT_PER & ";" & sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM & " ) >= 0; " & sBELEIHUNGSWERT_PER & ";" & sBELEIHUNGSWERT_WURDE_UEBERPRUEFT_AM & ")"
task.AppendField field
task.PerformTask
Set task = Nothing
Set db = Nothing
...
Set task = db.TableManagement
Set field = db.TableDef.NewField
field.Name = "JÜNGSTE_DATUM"
field.Description = "Hinzugefügtes Feld"
field.Type = WI_VIRT_DATE
field.Equation = "@if(@AGE(" & "JÜNGSTE_DATUM_PER_AM" & ";" & sBELEIHUNGSWERT_STATUS_PER & " ) >= 0; " & "JÜNGSTE_DATUM_PER_AM" & ";" & sBELEIHUNGSWERT_STATUS_PER & ")"
task.AppendField field
task.PerformTask
Set task = Nothing
Set db = Nothing
